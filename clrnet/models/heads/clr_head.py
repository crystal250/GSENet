...
from clrnet.models.losses.Gliou_loss import gliou_loss as liou_loss
# from clrnet.models.losses.lineiou_loss import liou_loss
from clrnet.models.losses.angle_loss import a_loss
...
class CLRHead(nn.Module):
    def __init__(self,
        ...
        # self.TLANlinear3 = nn.Linear(5120,1920)

        # self.linear = nn.Linear(2560,1920)
    ...
    def forward(self, x,x2, **kwargs):
        ...
        for stage in range(self.refine_layers):
            ...
            # ViTfea = self.ViT(x2)
            # ViTfea = self.linear(ViTfea)
            # ViTfea = ViTfea.reshape(batch_size * num_priors,10)
            #
            # CNNfea = self.CNN(x2)
            # CNNfea = self.linear(CNNfea)
            # CNNfea = CNNfea.reshape(batch_size * num_priors,10)
            #
            # MHAfea = self.MHA(x2)
            # MHAfea = self.linear(MHAfea)
            # MHAfea = MHAfea.reshape(batch_size * num_priors,10)
