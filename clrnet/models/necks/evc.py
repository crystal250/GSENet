...
class LightMLPBlock(nn.Module):
    ...
    def forward(self, x):
        if self.use_layer_scale:
            ...
            # print(self.dw(self.norm1(x)).shape)
            # print(self.drop_path(self.layer_scale_1.unsqueeze(-1).unsqueeze(-1) * self.dw(self.norm1(x))).shape)
...
class EVCBlock(nn.Module):
    ...
    def forward(self, x):
        #vit_cam
        identity=x
        x_vit=self.vit_multicam(x)
        x_vit +=identity

        x1 = self.maxpool(self.act1(self.bn1(self.conv1(x))))
        # LVCBlock
        # x_lvc = self.lvc(x1)
        # print(np.array(x_lvc.cpu().detach()).shape)
        # identity=x1
        # x_vit=self.vit_multicam(x1)
        # x_vit +=identity
        #vit_cam
        # LightMLPBlock
        x_lmlp = self.l_MLP(x1)
        x_lmlp +=identity
        # print(np.array(x_lmlp.cpu().detach()).shape)
        # concat
        x = torch.cat((x_vit, x_lmlp), dim=1)
        x = self.cnv1(x)
        return x